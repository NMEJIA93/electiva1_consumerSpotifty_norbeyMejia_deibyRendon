...
export const Hero = () => {
  ...
  // Filtrar playlists propias y compartidas por separado
  const filteredOwnPlaylists = filteredPlaylists.filter(playlist => 
    ownPlaylists?.some(own => own.id === playlist.id)
  );
  
  const filteredSharedPlaylists = filteredPlaylists.filter(playlist => 
    sharedPlaylists?.some(shared => shared.id === playlist.id)
  );

  // Mostrar solo las primeras 3 playlists propias si no está expandido
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <div className={`absolute -bottom-3 left-4 px-3 py-1 rounded-full text-sm font-bold ${
                    isDarkMode 
                      ? ownPlaylists?.some(own => own.id === selectedPlaylist.id) 
                    ...
                    {ownPlaylists?.some(own => own.id === selectedPlaylist.id) ? 'Tu playlist' : 'Compartida'}
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mt-4">
                  <div>
                    ...
                    <p className="text-sm font-medium">
                      <span className={isDarkMode ? 'text-gray-300' : 'text-gray-700'}>Canciones:</span> {selectedPlaylist.songs.length}
                    </p>
                  </div>
                  <p className="text-sm">
